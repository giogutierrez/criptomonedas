...
public class IdentificaMutanteUtil {
	...
	public static void addHorizontal(String[] dna){
		List<String> result = new ArrayList<>();
		  ...
		  Constantes.DNA_SECUENCIA.stream().filter(w->a.contains(w)).count() > 0)
		 .count();
	}
	...
	public static void addVertical(char[][] dnaMatriz) {
		...
		for (int fil = 0; fil < dnaMatriz.length; fil++) {
			for (int col = 0; col < dnaMatriz[fil].length; col++) {
				...
				if (datoIndividual.length() >= Constantes.SIZE_ADN) {
					if (validaCadena(datoIndividual.toString()).equals(Constantes.OK)) {
						if (cantidadSecuencia >= Constantes.SIZE_MUTANTE) {
							return;
						}
						break;
	...
	public static void addDiagonalUno(char[][] dnaMatriz) {
		...
		int altura = dnaMatriz.length, anchura = dnaMatriz[0].length;

		StringBuilder datoIndividual = new StringBuilder();

		for (
				// Recorre los inicios de cada diagonal en los bordes de la matriz.
				int diagonal = 1 - anchura; // Comienza con un número negativo.
			...
			for (
					...
					Integer vertical = Math.max(0, diagonal), horizontal = Math.max(0, diagonal); vertical < altura
			...
			// listaIndividual.add(datoIndividual.toString());
			datoIndividual = new StringBuilder();
		}

		// String[] datosVertical = new String[ listaIndividual.size() ];
		// listaIndividual.toArray( datosVertical );
		// return datosVertical;
	}
	...
	public static void addDiagonalDos(char[][] dnaMatriz) {
		...
		int n = dnaMatriz.length, 
			m = dnaMatriz[0].length, 
			cantDiagonales = n + m - 1;

		StringBuilder datoIndividual = new StringBuilder();
		List<String> listaIndividual = new ArrayList<String>();
